...
#
# Milvaion Maintenance Worker Docker Build Script
# Builds and pushes the Maintenance Worker Docker image to a registry.
#
# Usage:
#   ./build-maintenance-worker.sh -r milvasoft -t 1.0.0
#   ./build-maintenance-worker.sh -r ghcr.io/milvasoft -t latest --skip-push
#
...
DOCKERFILE="$ROOT_DIR/src/Workers/MilvaionMaintenance/Dockerfile"

echo ""
echo -e "${MAGENTA}===============================================================${NC}"
echo -e "${MAGENTA}        Milvaion Maintenance Worker Build Script${NC}"
echo -e "${MAGENTA}===============================================================${NC}"
...
IMAGE_NAME="$REGISTRY/milvaion-maintenance:$TAG"
...
if [ "$TAG" != "latest" ]; then
    ...
    docker tag "$IMAGE_NAME" "$REGISTRY/milvaion-maintenance:latest"
...
if [ "$SKIP_PUSH" = false ]; then
    ...
    if [ "$TAG" != "latest" ]; then
        print_info "Pushing $REGISTRY/milvaion-maintenance:latest..."
        docker push "$REGISTRY/milvaion-maintenance:latest"
    fi
...
echo "     docker-compose up -d maintenance-worker"
