...
<#
.SYNOPSIS
    Builds and pushes Milvaion SQL Worker Docker image.
...
.EXAMPLE
    .\build-sql-worker.ps1 -Registry "milvasoft" -Tag "1.0.0"

.EXAMPLE
    .\build-sql-worker.ps1 -Registry "ghcr.io/milvasoft" -Tag "latest" -SkipPush
#>
...
$dockerfile = Join-Path $rootDir "src/Workers/SqlWorker/Dockerfile"

Write-Host @"
===============================================================
           Milvaion SQL Worker Build Script
===============================================================
  Registry: $Registry
  Tag:      $Tag
...
$imageName = "$Registry/milvaion-sql-worker:$Tag"
...
if ($Tag -ne "latest") {
    ...
    docker tag $imageName "$Registry/milvaion-sql-worker:latest"
...
if (-not $SkipPush) {
    ...
    if ($Tag -ne "latest") {
        Write-Info "Pushing $Registry/milvaion-sql-worker:latest..."
...
Write-Host "     docker-compose up -d sql-worker" -ForegroundColor Gray
