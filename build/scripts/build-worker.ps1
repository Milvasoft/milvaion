...
<#
.SYNOPSIS
    Builds and pushes Milvaion Sample Worker Docker image.
...
.EXAMPLE
    .\build-worker.ps1 -Registry "ghcr.io/milvasoft" -Tag "latest" -SkipPush
#>
...
$dockerfile = Join-Path $rootDir "src/Workers/SampleWorker/Dockerfile"

Write-Host @"
===============================================================
           Milvaion Sample Worker Build Script
===============================================================
  Registry: $Registry
  Tag:      $Tag
...
$imageName = "$Registry/milvaion-sampleworker:$Tag"
...
if (-not $SkipPush) {
    ...
    if ($Tag -ne "latest") {
        Write-Info "Pushing $Registry/milvaion-sampleworker:latest..."
...
Write-Host "`n  Or scale with Docker Compose:" -ForegroundColor White
Write-Host "     docker-compose up --scale worker=4 -d" -ForegroundColor Gray
