...
<#
.SYNOPSIS
    Builds and pushes Milvaion HTTP Worker Docker image.
...
.EXAMPLE
    .\build-http-worker.ps1 -Registry "milvasoft" -Tag "1.0.0"

.EXAMPLE
    .\build-http-worker.ps1 -Registry "ghcr.io/milvasoft" -Tag "latest" -SkipPush
#>
...
$dockerfile = Join-Path $rootDir "src/Workers/HttpWorker/Dockerfile"

Write-Host @"
===============================================================
           Milvaion HTTP Worker Build Script
===============================================================
  Registry: $Registry
  Tag:      $Tag
...
$imageName = "$Registry/milvaion-http-worker:$Tag"
...
if ($Tag -ne "latest") {
    ...
    docker tag $imageName "$Registry/milvaion-http-worker:latest"
...
if (-not $SkipPush) {
    ...
    if ($Tag -ne "latest") {
        Write-Info "Pushing $Registry/milvaion-http-worker:latest..."
...
Write-Host "     docker-compose up -d http-worker" -ForegroundColor Gray
