...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Options:"
            echo "  -r, --registry    Docker registry (e.g., ghcr.io/milvasoft)"
            echo "  -t, --tag         Image tag (default: latest)"
            echo "  --skip-push       Build image but don't push"
            echo ""
            echo "Example:"
            echo "  $0 -r ghcr.io/milvasoft -t 1.0.0"
            exit 0
            ;;
...
DOCKERFILE="$ROOT_DIR/src/Milvaion.Api/Dockerfile"

echo -e "${CYAN}===============================================================${NC}"
echo -e "${CYAN}           Milvaion Scheduler API Build Script            ${NC}"
echo -e "${CYAN}===============================================================${NC}"
...
IMAGE_NAME="$REGISTRY/milvaion-api:$TAG"
...
if [ "$TAG" != "latest" ]; then
    ...
    docker tag "$IMAGE_NAME" "$REGISTRY/milvaion-api:latest"
...
if [ "$SKIP_PUSH" = false ]; then
    ...
    if [ "$TAG" != "latest" ]; then
        info "Pushing $REGISTRY/milvaion-api:latest..."
        docker push "$REGISTRY/milvaion-api:latest" || error "Docker push (latest) failed"
    fi
...
echo -e "     docker run -d -p 5000:5000 $IMAGE_NAME"
