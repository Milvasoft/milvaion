...
#
# Milvaion HTTP Worker Docker Build Script
# Builds and pushes the HTTP Worker Docker image to a registry.
#
# Usage:
#   ./build-http-worker.sh -r milvasoft -t 1.0.0
#   ./build-http-worker.sh -r ghcr.io/milvasoft -t latest --skip-push
#
...
DOCKERFILE="$ROOT_DIR/src/Workers/HttpWorker/Dockerfile"

echo ""
echo -e "${MAGENTA}===============================================================${NC}"
echo -e "${MAGENTA}           Milvaion HTTP Worker Build Script${NC}"
echo -e "${MAGENTA}===============================================================${NC}"
...
IMAGE_NAME="$REGISTRY/milvaion-http-worker:$TAG"
...
if [ "$TAG" != "latest" ]; then
    ...
    docker tag "$IMAGE_NAME" "$REGISTRY/milvaion-http-worker:latest"
...
if [ "$SKIP_PUSH" = false ]; then
    ...
    if [ "$TAG" != "latest" ]; then
        print_info "Pushing $REGISTRY/milvaion-http-worker:latest..."
        docker push "$REGISTRY/milvaion-http-worker:latest"
    fi
...
echo "     docker-compose up -d http-worker"
