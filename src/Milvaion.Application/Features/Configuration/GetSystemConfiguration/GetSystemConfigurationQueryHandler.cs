...
public class GetSystemConfigurationQueryHandler(IConfiguration configuration, MilvaionConfig milvaionConfig) : IInterceptable, IQueryHandler<GetSystemConfigurationQuery, SystemConfigurationDto>
{
    ...
    /// <summary>
    /// Reads live resource usage for the configuration payload.
    /// </summary>
    /// <remarks>
    /// The collection itself lives in <see cref="SystemResourceReader"/>, because the resource usage endpoint
    /// needs the same numbers and two copies would have drifted apart the first time either was corrected. The
    /// returned shape is unchanged - the reader's richer sample is mapped back down to the DTO this payload has
    /// always carried.
    /// </remarks>
    private static SystemResourcesDto GetSystemResources() => SystemResourceReader.ToSystemResources(SystemResourceReader.Read());
